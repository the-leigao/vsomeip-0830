...
std::shared_ptr<endpoint> endpoint_manager_base::create_local(client_t _client) {
    //VSOMEIP_INFO << "endpoint_manager_base create_local," << _client;
...
std::shared_ptr<endpoint> endpoint_manager_base::create_local_unlocked(client_t _client) {
    ...
    {
        //std::cout << "_client != VSOMEIP_ROUTING_CLIENT,local_endpoints_[_client] = its_endpoint" << std::endl;
        local_endpoints_[_client] = its_endpoint;
    }
    rm_->register_client_error_handler(_client, its_endpoint);
    //std::cout << "register_client_error_handler," << _client << std::endl;
...
{   
    // VSOMEIP_INFO << "endpoint_manager_base,find_local_unlocked," << _client;
    // for (auto iter = local_endpoints_.begin(); iter != local_endpoints_.end(); iter++)
    // {
    //     VSOMEIP_INFO << "key," << iter->first << ",value," << iter->second;
    // }
