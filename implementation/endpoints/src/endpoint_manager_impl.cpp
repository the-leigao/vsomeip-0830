...
std::shared_ptr<endpoint> endpoint_manager_impl::create_server_endpoint(
    ...
    try {
        ...
        if (_start) {
            if (_reliable) 
            {
                //VSOMEIP_INFO << "std::make_shared<tcp_server_endpoint_impl,port," << _port << ",address," << its_unicast.to_v4();
            ...
            {
                //VSOMEIP_INFO << "std::make_shared<udp_server_endpoint_impl,port," << _port << ",address," << its_unicast.to_v4();
