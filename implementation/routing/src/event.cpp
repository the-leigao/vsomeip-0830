        ...
        bool _force) 
{
    ...
    {
        VSOMEIP_INFO << "Can't set payload for event " << std::hex << message_->get_method() << " as it isn't provided";
    }
}
...
void event::notify_one_unlocked(client_t _client,
    ...
    if (_target) 
    {
        if (is_set_) 
        {
            ...
            // }
            //VSOMEIP_INFO << "routing_->send_to(_client, _target, message_)";
            ...
            // }
            //VSOMEIP_INFO << "event::set_session time," << (t2 - t1) * millisecond_coefficient << " ms" << std::endl;
            //VSOMEIP_INFO << "event::send_to time," << (t3 - t2) * millisecond_coefficient << " ms";
        }
        else 
        {
            VSOMEIP_INFO << __func__
...
bool event::set_payload_helper(const std::shared_ptr<payload> &_payload, bool _force) {
    ...
    if (!is_change) 
    {
        ...
        {
            is_change = epsilon_change_func_(its_payload, _payload);
        }
    }
    return is_change;
}
