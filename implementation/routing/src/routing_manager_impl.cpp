...
bool routing_manager_impl::send_to(
        ...
        std::shared_ptr<message> _message) 
{
    // VSOMEIP_INFO << "routing_manager_impl,send_to_1";
    ...
    // }
    // VSOMEIP_INFO << "routing_manager_impl.send_to_1 time," << (t2 - t1) * millisecond_coefficient << " ms";
    return (is_sent);
}
...
{
    //VSOMEIP_INFO << "routing_manager_impl,send_to";

    // double millisecond_coefficient = 1000.0 / 1000000000ull;
    // ulong t1 = 0, t2 = 0, t3 = 0;
    // {
    //     timespec time;
    //     clock_gettime(CLOCK_MONOTONIC_RAW, &time);
    //     t1 = time.tv_nsec + (ulong)time.tv_sec * 1000000000ull;
    ...
    if (its_endpoint) 
    {
        ...
        (void) _instance;
#endif
        bool retv = its_endpoint->send_to(_target, _data, _size);
        // {
        //     timespec time;
        //     clock_gettime(CLOCK_MONOTONIC_RAW, &time);
        //     t3 = time.tv_nsec + (ulong)time.tv_sec * 1000000000ull;
        // }
        //VSOMEIP_INFO << "routing_manager_impl.find_server_endpoint time," << (t2 - t1) * millisecond_coefficient << " ms";
        //VSOMEIP_INFO << "routing_manager_impl.send_to time," << (t3 - t2) * millisecond_coefficient << " ms";
        ...
        ) 
{
    ...
    {
        //std::cout << "routing_manager_impl notify_one find_local false" << std::endl;
        ...
        if (its_event) 
        {
            ...
            //std::cout << "its_targets size," << its_targets.size() << std::endl;
